...
- **Dead-mic detection.** Catches the common case where `AVAudioEngine` starts but no audio actually flows (e.g. built-in mic with the lid closed — the mic sits in the lid hinge). If no signal is observed in the first ~2s, the HUD swaps to an orange "No audio — check microphone" warning and a Sosumi alert chime plays. Noise-cancelled inputs (AirPods, Continuity iPhone mic) won't false-positive — they show signal on engine start, which disables the check for the rest of the take. On stop, the `.caf` file is verified to exist and be non-empty before the transcription path runs, so a dead recording surfaces as a real error instead of silently producing an empty transcript.
