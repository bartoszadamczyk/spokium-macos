...
- A bug or supply-chain compromise in Spokium or its dependencies (whisper.cpp, `KeyboardShortcuts`, swift runtime) cannot read files outside the app's container.
...
Spokium runs under macOS App Sandbox. The entitlements file (`Spokium/Spokium.entitlements`) is the complete, audited list:
...
- **Filesystem** — Spokium can only read/write its container at `~/Library/Containers/com.bartoszadamczyk.Spokium/...` plus user-selected paths via the open/save panels. It cannot read your home directory, your keychain, other apps' containers, or system files outside `/System` and `/Library/Frameworks`.
