...
- **Audio** — recorded to a `.caf` file in the sandbox tmp directory, transcribed locally, and deleted on normal success, cancel, start failure, audio-configuration failure, and transcription-task completion paths. `AppDelegate.cleanStaleTempFiles()` removes old `whisper-*.caf` temp files on next launch. Quit while actively recording or transcribing is a known cleanup-hardening area in `APP_REVIEW_IDEAS.md`.
